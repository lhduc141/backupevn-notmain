...
const CustomerLookupElectricityConsumptionAndPayment = () => {
  ...
  useEffect(() => {
    getPeriods({ customerCode: String(customer?.customerCode) })
      ...
      .then((rs: { data: PeriodType[] }) => {
        const currentMonth = rs.data[0]?.month || new Date().getMonth();
        const currentYear = new Date().getFullYear();
        const threeMonth: { month: number; year: number }[] = [];

        for (let i = 0; i < 3; i++) {
          const month = ((currentMonth - i + 11) % 12) + 1;
          const year = currentYear - (currentMonth - i <= 0 ? 1 : 0);
          threeMonth.push({ month, year });
        }

        const filteredData: customerPeriod[] = rs.data
          .filter((item) => threeMonth.some((monthObj) => item.month === monthObj.month && item.year === monthObj.year))
          .map((item) => ({
  ...
  useEffect(() => {
    if (isPeriodsFetched && periodsArr.length > 0) {
      ...
      Promise.all(promises).then((results) => {
        const sortedArr = results
          .sort((a, b) => {
            const indexA = periodsArr.findIndex(
              (period) => period.month === a.item.month && period.year === a.item.year
            );
            const indexB = periodsArr.findIndex(
              (period) => period.month === b.item.month && period.year === b.item.year
            );
            return indexA - indexB;
          })
          .map((result) => result.data as PowerComsumption);
...
export default CustomerLookupElectricityConsumptionAndPayment;
