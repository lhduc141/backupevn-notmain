...
import { useShiftsUser } from 'hooks';
import { groupBy } from 'lodash';
import { shiftsMessages } from 'messages';
import { colorsBase } from 'themes';
import { TIME_STAMP_FORMAT, WEEKDAY_FORMAT } from 'utils';

type UserShiftTabProps = {
  userId: number;
};
const UserShiftTab = ({ userId }: UserShiftTabProps) => {
  const { userShift, isLoading, handleLoadMore, hasMore } = useShiftsUser(userId);
  const groupedShifts = groupBy(userShift, (o) => dayjs(o.createdAt).format(WEEKDAY_FORMAT));
